...
Gaussian fitting is manual-first: exactly two region markers and at least one peak marker are required. Press `P` to place markers individually or `O` to populate editable markers with the original peak finder; detection never starts a fit. A non-`None` background also requires explicit background windows. The fit-results table pairs Initial min/guess/max values for position, FWHM, and net height with the fitted Temp values and uncertainties. Editing one parameter holds the other two fixed, and the displayed ranges are passed to one deterministic composite solve. The plot shows only a translucent width/height envelope; position bounds are edited in the table and are not drawn. Fit-quality metrics are objective-aware and advisory, so they never prevent storing a completed temporary fit.
