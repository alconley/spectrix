...
**Spectrix** is an **AI-assisted nuclear spectrum analysis** and experimental event-data exploration tool. It provides tools for histogramming, Gaussian fitting, interactive visualization of 1D and 2D histograms, and source-grounded help through **Spectrix AI**.
...
## Getting Started with Spectrix AI

Spectrix includes an AI assistant for usage questions and source-grounded workflow help. The assistant is currently Ollama-only in the UI.

### Setup

1. Install and start [Ollama](https://ollama.com/).
2. Sign in to Ollama if you want to use the default cloud model:

```bash
ollama signin
```

3. Spectrix uses this model by default:

```text
qwen3-coder:480b-cloud
```

This is the model used while testing the Spectrix AI workflow.

4. If you want a local model instead, pull one with Ollama:

```bash
ollama pull <model-name>
```

5. Start Spectrix and open the assistant with **Open AI Assistant** from the left panel or from the **Processor -> Analysis** area.

### AI Setup in Spectrix

In the **AI Setup** section:

- **Provider** is fixed to **Ollama**.
- **Ollama URL** defaults to `http://127.0.0.1:11434`.
- **Model** defaults to `qwen3-coder:480b-cloud`.
- **Refresh Ollama Models** lists installed local models only.
- You can type another Ollama model name manually.
- Use [Ollama popular models](https://ollama.com/library?sort=popular) to find other cloud or local model names.

### Current AI Capabilities

Spectrix AI is designed to answer questions about how Spectrix works before it performs analysis for you. When you ask a question, it searches the Spectrix Rust source code first, then answers from the implementation instead of guessing from generic documentation.

It can currently help with:

- Explaining Spectrix UI workflows, such as creating 1D cuts, 2D cuts, histograms, derived columns, and fits.
- Tracing which buttons, menus, panels, and plot interactions are involved in a task.
- Explaining what a feature means for event data, such as how cuts create event-selection masks or how histograms bin energy, time, position, PID, or calibrated derived values.
- Using the current session context, including selected files, loaded columns, derived columns, defined cuts, active histogram cuts, and defined histograms.
- Keeping a scrollable conversation that can be cleared when you want to start fresh.

The AI does **not** currently:

- Click buttons, change Spectrix state, create histograms, create cuts, or run fits for you.
- Perform autonomous physics analysis or cross-section extraction.
- Replace checking the actual plots, fit results, gates, calibrations, and saved outputs.

For now, treat Spectrix AI as an implementation-aware guide and analysis companion. Direct AI-driven analysis and app actions are planned for later.

---

## Overview

The **Spectrix** program is an AI-assisted spectrum analysis environment that reads `.parquet` files using the [Polars](https://docs.rs/polars/latest/polars/) crate.  
