...
Use **Defaults** in the top bar to open the searchable, collapsible app-level defaults panel. It controls new-generation settings for 1D and 2D plots, fits, markers, peak finding, cuts, projections, heatmaps, and the shared histogram tile layout. Histogram defaults are reapplied when parquet histograms are calculated or ROOT histograms are imported, while right-click histogram settings remain available for temporary per-plot overrides until the next generation. Memory and AI connection/model settings are shared live across sessions; the **New Sessions** section controls only the initial state of subsequently created or reset sessions.
...
Background parameters vary jointly with Gaussian peaks by default, including their cross-covariance in the total fit uncertainty. After explicitly fitting a background, **Lock manual background** can hold those coefficients fixed for subsequent peak fits. Python remains in Spectrix for unrelated features such as uproot, but the production fitter never falls back to Python.

Gaussian fitting is manual-first: exactly two region markers and at least one peak marker are required. Press `P` to place markers individually or `O` to populate editable markers with the original peak finder; detection never starts a fit. A non-`None` background also requires explicit background windows. The fit-results table places editable Initial min/guess/max values for position, FWHM, and net height beside fitted values and uncertainties in the same peak row. Fitted measurements use PDG-style scientific notation. Editing one parameter holds the other two fixed, and the displayed ranges are passed to one deterministic composite solve. The plot shows only a translucent width/height envelope; position bounds are edited in the table and are not drawn. Fit-quality metrics are objective-aware and advisory, so they never prevent storing a completed temporary fit.
...
| **F** | Fit Gaussians | Fits explicitly marked peaks and the unlocked background jointly in the selected region. |
...
- Gaussian fitting never inserts peaks automatically. A significant isolated positive residual is shown as a possible-unmarked-peak warning so the user can add the intended marker and refit.
- Each peak starts from its exact marker position, a background-subtracted height, an interpolated/fallback FWHM, and positive local area. Center ownership cells are bounded by adjacent-marker midpoints.
- Shared FWHM uses the strongest clean marked peak as its initial width; Independent FWHM keeps per-peak estimates.
- Position, FWHM, and net-height bounds remain visible and editable. The fitter varies height directly, which keeps height and width controls independent while retaining integrated-bin amplitude/area output.
