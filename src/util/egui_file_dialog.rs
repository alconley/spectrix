...
impl FileDialog {
                                ...
                                if response.double_clicked() {
                                    match self.dialog_type {
                                        ...
                                        DialogType::OpenFile => {
                                            if info.is_dir() {
                                                command =
                                                    Some(Command::BrowseDirectory(info.clone()));
                                            } else if (self.filename_filter)(
                                                self.filename_edit.as_str(),
                                            ) {
    ...
    pub fn ui_embeded(&mut self, ui: &mut Ui) {
        enum Command {
            Open(FileInfo),
            OpenSelected,
            BrowseDirectory(FileInfo),
        ...
        // Top directory field with buttons.
        egui::TopBottomPanel::top("egui_file_top_new").show_inside(ui, |ui| {
            ui.horizontal(|ui| {
                ui.add_enabled_ui(self.path.parent().is_some(), |ui| {
                    let response = ui
                        .button("⬆")
                                    ...
                                    match self.dialog_type {
                                        ...
                                        DialogType::OpenFile => {
                                            if info.is_dir() {
                                                ...
                                                //   command = Some(Command::Open(info.clone())); // AC: Commented out to prevent opening files on double click
                                            }
                                        }
                                        DialogType::SaveFile => {
                                            if info.is_dir() {
    ...
    pub fn selected_file_paths(&self) -> Vec<PathBuf> {
    ...
    pub fn set_filter(&mut self, suffix: &str) {
        let suffix_owned = suffix.to_string();
        self.filename_filter = Box::new(move |file_name: &str| file_name.ends_with(&suffix_owned));
        self.refresh();
    }
