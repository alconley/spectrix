...
fn source_backed_gaussian_fit_fallback_response(prompt: &str) -> Option<AiResponse> {
    ...
    Some(AiResponse {
        summary: "Gaussian fitting in Spectrix is manual-first. Place exactly two region markers with `R`, then place every intended peak with `P`, or press `O` to populate editable markers with the original peak finder. Choose the explicit background family in `Fits`; any non-`None` background also requires one or more background windows placed with `B`. Press `F` to run the fit; detection never launches a fit or chooses a model.\n\nEach fit-results row places Initial and Temp values side by side. Initial values expose editable min/guess/max controls for position, FWHM, and background-subtracted height; fitted Temp measurements use PDG-style scientific notation with their uncertainties. Editing one parameter holds the other displayed variables fixed. The plot shows only a translucent envelope spanning the width/height ranges; position bounds are not drawn. Shared FWHM applies one width to every peak while preserving each height; Independent FWHM permits individual widths. The displayed values and ranges are passed to one deterministic composite solve.\n\nA successful result becomes the temporary fit and its fitted centers, FWHM values, and heights become the new handles. Fit-quality metrics are advisory and never block `S` or Store Fit. Press `S` or Store Fit to keep it. Stored fits are not changed by later handle edits.".to_owned(),
        clarification_questions: Vec::new(),
        notes: vec![
