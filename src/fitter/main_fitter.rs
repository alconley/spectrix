...
pub enum FitModel {
    Gaussian(Vec<f64>, bool, bool, bool, bool, f64), // put the initial peak locations in here, free sigma, free position
...
impl Fitter {
    ...
    pub fn get_peak_markers(&self) -> Vec<f64> {
        ...
        } else if let FitModel::Gaussian(peak_markers, _, _, _, _, _) = &self.model {
            peak_markers.clone()
        } else {
            Vec::new()
        }
