...
fn default_plot_bounds(x_min: f64, x_max: f64, y_min: f64, y_max: f64) -> bool {
    (bounds_match(x_min, -1.0) || bounds_match(x_min, 0.0))
        && bounds_match(x_max, 1.0)
        && bounds_match(y_min, 0.0)
        && bounds_match(y_max, 1.0)
}
...
impl Histogram {
    ...
    fn display_x_range(&self) -> (f64, f64) {
    ...
    fn non_auto_y_bounds(&self) -> (f64, f64) {
    ...
    fn full_y_bounds(&self) -> (f64, f64) {
        if self.plot_settings.auto_fit_y_to_visible_range {
            self.auto_fit_y_bounds(self.range.0, self.range.1)
        } else {
            self.non_auto_y_bounds()
        }
    }

    fn full_plot_bounds(&self) -> egui_plot::PlotBounds {
        let range = self.display_x_range();
        let (y_min, y_max) = self.full_y_bounds();
    ...
    pub fn limit_scrolling(&mut self, plot_ui: &mut egui_plot::PlotUi<'_>) {
        ...
        let (y_min, y_max) = self.non_auto_y_bounds();

        if default_plot_bounds(current_x_min, current_x_max, current_y_min, current_y_max) {
...
mod tests {
    ...
    fn recognizes_egui_default_1d_plot_bounds() {
        assert!(default_plot_bounds(-1.0, 1.0, 0.0, 1.0));
        assert!(default_plot_bounds(0.0, 1.0, 0.0, 1.0));
    }
